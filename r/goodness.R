...
NSeff2 <- function (q, col.obs="obs", col.pred="pred") {
     if (!(col.obs %in% names(q) & col.pred %in% names(q) ))
          stop( "Must provide data frame with observed and predicted columns")
     
     q <- q[ !is.na(q[,col.obs]) & !is.na(q[,col.obs]), c(col.obs, col.pred)]
     names(q) <- c("obs","pred")
#      if (length(Qob))
#      Qsim <- Qsim[!is.na(Qobs) & !is.na(Qsim)]
#      Qobs <- Qobs[!is.na(Qobs) & !is.na(Qsim)]

     if (nrow(q)==0) 
          return(NA)
     m <- mean(q$obs)
     NS <- 1 - (sum((q$obs - q$pred)^2)/sum((q$obs - m)^2))
     return(NS)
}
