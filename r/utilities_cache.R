...
#                              object=NULL, is.rdata=T ) {
#      
#      
#      # a <- "ftp://ftp.hydro.washington.edu/pub/blivneh/CONUS/Meteorology.asc.v.1.2.1915.2011.bz2"
#      # b <- "ftp://gdo-dcp.ucllnl.org/pub/dcp/archive/OBS/livneh2014.1_16deg/ascii/daily"    
#      # c <- "http://felek.cns.umass.edu:9283/data"
# 
#      server.name <- colsplit( server.url, pattern = "/", names = 1:10 )[[3]]
#      server.name <- colsplit( server.name, pattern = ":", names = 1:10 )[[1]]
#      server.name
#      server.url<-b
#      
...
#      
# 
#      if ( !( file %in% list.files(path=file.path(cache.dir.global,"data",server.name,dir) ) ) ) {
#           if ( message == "default" )
#                cat( paste0("Downloading file ",file,". (This will be cached locally for future use.)\n") )
#           else if ( !is.null(message) )     
#                cat(message)
